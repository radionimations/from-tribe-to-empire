...
const HISTORICAL_EVENTS = [
  ...
  { year: 2150, civ: { name: "Antarctic Temperian Empire", lat: 0.0, lon: 0.0, color: "#cfe4ff", planet: "Triton" },
    message: "Antarctic Temperian Empire [ATE] proclaimed - cold-climate colonists from Earth's poles unite Triton's nitrogen plains as the first true off-world empire" },
  { year: 2200, civ: { name: "The New Space Imperium of Flar'", lat: 0.0, lon: 0.0, color: "#e84a3a", planet: "Proxima Centauri b" },
    message: "New Space Imperium of Flar' [NSIoF] proclaimed - the second great off-world polity unifies under an imperial charter on Proxima b" },
  { year: 2310, civ: { name: "Orbital Exploration Agency", lat: 0.0, lon: 0.0, color: "#5dc4e8", planet: "Mercury" },
  ...
  { year: 2410, civ: { name: "Chillax Space Administration", lat: 0.0, lon: 0.0, color: "#7ec3ed", planet: "Asteroid Belt" },
...
window.expandAllColonizers = function () {
  ...
  for (const army of colonizers) {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        if (grid[r][c] === player.id) ownTiles.push([c, r]);
      }
    }
    if (ownTiles.length === 0) continue;
    const sample = [];
    const stride = Math.max(1, Math.floor(ownTiles.length / army.count));
    for (let i = 0; i < army.count && sample.length < army.count; i++) {
      const idx = (i * stride) % ownTiles.length;
      sample.push(ownTiles[idx]);
    }
    if (sample.length > 0) {
      const [c0, r0] = sample[0];
      army.col = c0; army.row = r0;
      army.prevCol = c0; army.prevRow = r0;
    }
    for (let i = 1; i < sample.length; i++) {
      const [c, r] = sample[i];
      player.armies.push({
        id: nextArmyId++, col: c, row: r,
        type: "colonizer", count: 1, civId: player.id, moves: 1, planet,
      });
      army.count--;
    }
  }
